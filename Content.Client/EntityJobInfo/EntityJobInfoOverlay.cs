...
public sealed class EntityJobInfoOverlay : Overlay
{
    ...
    protected override void Draw(in OverlayDrawArgs args)
    {
        ...
        foreach (var mob in _entManager.EntityQuery<MobStateComponent>(true))
        {
            ...
            var icon = "NoId";
            var icon_job = GetIcon(mob.Owner);
            if (icon_job != null)
                icon = icon_job;

            var sprite_icon = new SpriteSpecifier.Rsi(new ResourcePath("/Textures/Interface/Misc/job_icons.rsi"), icon);
            var _barTexture = _entManager.EntitySysManager.GetEntitySystem<SpriteSystem>().Frame0(sprite_icon);
            ...
            {
                yOffset = sprite.Bounds.Height + 7f;
                xOffset = sprite.Bounds.Width + 7f;
            }
            else
            {
                yOffset = 1f;
